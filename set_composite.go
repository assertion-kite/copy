...
func setCompositeMap(dstType, srcType reflect.Type, dst, src unsafe.Pointer, opt options, of *offsetAndFunc) (err error) {
	...
	for iter.Next() {
		...
		if isBaseKeyType {
			...
			if err = copyInner(newDstKey.Interface(), srcMapKey.Interface(), opt); err != nil {
				return err
			}
		}
		...
		if err = copyInner(newDstVal.Interface(), srcMapVal.Interface(), opt); err != nil {
			return err
		}
	next:
...
func setCompositeSlice(dstType, srcType reflect.Type, dst, src unsafe.Pointer, opt options, of *offsetAndFunc) error {
	...
	return copyInner(dstValPtr.Interface(), srcValPtr.Interface(), opt)
}
